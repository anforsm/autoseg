...
if __name__ == "__main__":
    ...
    input_zarr = (
        "/home/anton/.cache/autoseg/datasets/SynapseWeb/kh2015/data/oblique.zarr"
    )

    output_zarr = os.path.join(
        os.path.dirname(input_zarr), "2d_" + os.path.basename(input_zarr)
    )

    # datasets = [i for i in os.listdir(sys.argv[1]) if '.z' not in i] # all datasets in input_zarr

    # datasets = sys.argv[2:]
    datasets = ["raw/s0", "labels/s0", "labels_mask/s0"]
    print(datasets)
    ...
    for dataset in datasets:
        ...
        write_roi = Roi(roi.offset[1:], roi.shape[1:])
        write_vs = vs[1:]

        args = ({"index": index, "data": section} for index, section in enumerate(data))
