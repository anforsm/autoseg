...
def agglomerate_in_block(
    affs, fragments, rag_provider, block, merge_function, threshold
):
    logging.info(
    ...
    fragments_relabelled, n, fragment_relabel_map = relabel(
        fragments, return_backwards_map=True
    )
    ...
    if affs.shape[0] == 2:
        ...
        affs = np.stack([np.ones_like(affs[0]), affs[-2], affs[-1]])
    ...
    smoothed_affs = (gaussian_filter(affs, sigma=(0, 1, 2, 2)) - 0.5) * 0.05
