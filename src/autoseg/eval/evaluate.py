...
class EvaluateAnnotations:
    ...
    def evaluate(self):
        ...
        best_results = {"best_nvi": results[best_nvi_thresh]}
        ...
        if self.compute_mincut_metric:
            ...
            # }
            best_results["best_edits"] = results[best_edits_thresh]
    ...
    def evaluate_threshold(self, threshold):
        ...
        rand_voi_report = self.compute_rand_voi(threshold, return_cluster_scores=False)
...
def parse_str(input_string):
    ...
    # pred_dataset = '_'.join(parts[0:2])
    # normalize_preds = parts[2].replace("Norm", "")
    # background_mask = parts[3].split("BoundaryMask")[0]
    # mask_thresh = int(parts[3].split("BoundaryMask")[1]) / 100.0
    # min_seed_distance = int(parts[4].replace("MinSeedDist", ""))
    # filter_fragments_value = int(parts[5].replace("FragFilter", "")) / 100.0
    pred_dataset = "blockwise/affs"
    normalize_preds = False
    background_mask = False
    mask_thresh = 0.5
    min_seed_distance = 15
    filter_fragments_value = 0.1
