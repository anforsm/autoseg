...
"""UNext 8 but extra act after second 1x1 pw conv AND after first dw conv. (end of each convpass). Also no patchify"""
DOWNSAMPLE_FACTOR = 2
...
class UNeXt(UNeXtBase):
    def __init__(self, **kwargs):
        print(kwargs)
        super().__init__(**kwargs, debug=False)
        in_channels = kwargs["in_channels"]
        num_fmaps = kwargs["num_fmaps"]
        fmap_inc_factor = kwargs["fmap_inc_factor"]

        self.enc1 = nn.Sequential(
            nn.Conv3d(in_channels, num_fmaps, kernel_size=(1, 1, 1), stride=(1, 1, 1)),
